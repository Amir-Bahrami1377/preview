...
function CreateBlackIp(Id) {
    ...
    var url = Id > 0 ? "EditBlackIp" : "CreateBlackIp";

    ShowModal(`/Setting/IPBlockList/${url}`, model);
}

function DeleteBlackIp(id) {
    $.ajax({
        type: "GET",
        url: `/Setting/IPBlockList/DeleteRow`,
        data: { id: id },
...
function IpFormValidatoin() {
    ...
    var url = "CreateBlackIpSubmit";
    if (Identity > 0)
        url = "EditBlackIpSubmit";

    $.ajax({
        type: "POST",
        url: `/Setting/IPBlockList/${url}`,
        data: model,
        contentType: false,
        processData: false,
        success: function (data) {
